...
fun NavGraphBuilder.wallpaperComposable(navigateToHomeScreen: () -> Unit,wallpaperViewModel: WallpaperViewModel) {
    composable(route = "wallpaper/{imageId}", arguments = listOf(navArgument("imageId") {
        type = NavType.StringType
    })) { backStackEntry ->
        val imageId = backStackEntry.arguments?.getString("imageId") ?: ""
        WallpaperScreen(imageId = imageId, navigateBack = navigateToHomeScreen,wallpaperViewModel = wallpaperViewModel)
    }
}
